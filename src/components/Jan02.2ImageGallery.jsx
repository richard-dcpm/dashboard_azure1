...
export default function ImageGallery() {
  ...
  useEffect(() => {
    ...
    mapImages.forEach((img) => {
      ...
      // Build DOM popup node; preload image to decide between <img> or text fallback
        ...
        try {
          ...
          if (res.ok) {
            popupEl.innerHTML = `
              <div style="text-align:center;">
                <img src="${img.url}" style="max-width:250px;height:auto;border-radius:4px;margin-bottom:8px;" />
            ...
            popupEl.innerHTML = `
              <div style="text-align:center;max-width:260px;word-wrap:break-word;">
                <div style="font-size:12px;color:#333;">${img.url}</div>
                <div style="font-size:12px;color:#333;font-weight:600;">${img.name}</div>
                <div style="font-size:11px;color:#555;">${img.lat.toFixed(6)}, ${img.lon.toFixed(6)}</div>
                <div style="font-size:11px;color:#aa0000;margin-top:6px;">HTTP ${res.status}</div>
              </div>
            `;
          }
        } catch {
          popupEl.innerHTML = `
            <div style="text-align:center;max-width:260px;word-wrap:break-word;">
              ...
              <div style="font-size:12px;color:#333;font-weight:600;">${img.name}</div>
              <div style="font-size:11px;color:#555;">${img.lat.toFixed(6)}, ${img.lon.toFixed(6)}</div>
              <div style="font-size:11px;color:#aa0000;margin-top:6px;">Fetch error</div>
            </div>
          `;
        }
      })();

      L.marker([img.lat, img.lon]).addTo(map).bindPopup(popupEl, { maxWidth: 280 });
    });
        ...
        <div>
          ...
          {availableContainers.length > CONTAINERS_PER_PAGE && (
            <div className="flex justify-end gap-2 mt-4">
              ...
              <button
                disabled={(containerPage + 1) * CONTAINERS_PER_PAGE >= availableContainers.length}
                onClick={() => setContainerPage((p) => p + 1)}
                className={`${glass} px-4 py-2 disabled:opacity-40`}
              >
                Next
