...
**/
  ...
  const filteredFolders = useMemo(() => {
	  let rows = folders;
	  rows = rows.filter(n => !n.startsWith(TRASH_PREFIX));           // always strip
	  if (!showHidden) rows = rows.filter(n => !n.startsWith(HIDDEN_PREFIX)); // conditional
	  if (searchQuery.trim()) {
		const q = searchQuery.trim().toLowerCase();
		rows = rows.filter(n => n.toLowerCase().includes(q));
	  }
...
**/
  const renameFolder = async (name) => {
	...
	try {
		...
		if (err.message === "LEGACY_FOLDER") {
		  pushToast("success", `"${safe}" created. Delete "${name}" manually in Storage Explorer.`, 6000);
		  await loadFolders(activeContainer, currentPath);
		} else {
  ...
  const hideFolder = async (name) => {
    ...
    // Add handler alongside renameFolder / hideFolder
	const trashFolder = async (name) => {
	  if (!window.confirm(`Permanently hide "${name}"? It won't be visible anywhere in the UI.`)) return;
	  if (typeof trashFolderApi !== "function") return pushToast("error", "trashFolderApi not connected.");
	  try {
		await trashFolderApi(activeContainer, currentPath, name);
		pushToast("success", `"${name}" permanently hidden.`);
		await loadFolders(activeContainer, currentPath);
	  } catch (err) {
		console.error("[trashFolder] failed →", err);         // ← add this
		pushToast("error", `Failed: ${err.message}`);         // ← surface real message
	  }
	};
...
**/
  ...
  const enqueue = async (files) => {
	  ...
	  // ── Auto-folder guard: if uploading to container root, create a default folder ──
	  if (!targetPath) {
		const stamp = new Date().toISOString().slice(0, 16).replace("T", "_").replace(":", "-");
		const defaultFolder = `uploads_${stamp}`;    // e.g. "uploads_2025-07-10_14-35"

		try {
		  await createFolderApi(activeContainer, [], defaultFolder);
		  targetPath = `${defaultFolder}/`;
		  pushToast("success", `No folder selected — files placed in "${defaultFolder}".`);
		  // Refresh the folder list so the new folder appears
		  await loadFolders(activeContainer, currentPath);
		} catch {
		  return pushToast("error", "Failed to create default folder.");
		}
	  }

	  enqueueFiles(files, activeContainer, targetPath);
	  if (basePath) pushToast("success", `${files.length} file(s) queued.`); // avoid double toast
	  setProgressExpanded(true);
	};
