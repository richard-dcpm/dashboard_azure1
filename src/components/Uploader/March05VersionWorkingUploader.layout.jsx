...
import { Home as HomeIcon, Eye, EyeOff, Folder, ChevronDown, ChevronUp } from "lucide-react";
...
export default function UploaderLayout({
  ...
  // Controls whether the per-file breakdown is expanded below the overall bar
  ...
  const allDone    = queue.length > 0 && queue.every((i) => i.status === "complete" || i.status === "failed");
  ...
  const enqueue = (files) => {
    ...
    setProgressExpanded(true); // auto-expand when new files are added
  };
    ...
    <div className="glass-card p-8 max-w-7xl mx-auto mt-8">
      ...
      {!activeContainer ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-1 glass-card p-6 flex flex-col gap-4">
              ...
              <div
                ...
                className={`border-2 border-dashed rounded-lg p-10 text-center cursor-pointer transition flex-1
          ...
          {/* ── Inline progress section (below grid, only when queue has items) ── */}
          {hasQueue && (
            <div className="mt-4 glass-card border border-glass-border rounded-xl overflow-hidden">

              {/* ── Overall bar row — always visible, click to expand ── */}
              <button
              ...
              >
                ...
                <span className="text-sm font-semibold text-fg/80 shrink-0">
                  {allDone
                    ? `✓ All uploads complete (${totalCount})`
                    ...
                    return (
                      <li key={item.id} className="py-2.5">
                        ...
                        <div className="flex justify-between items-center mb-1.5">
                          <span className="text-xs text-fg/80 truncate max-w-[60%] font-mono" title={item.blobPath}>
                            {item.name}
                          </span>
