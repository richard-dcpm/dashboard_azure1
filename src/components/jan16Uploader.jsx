...
export default function Uploader() {
  ...
  // Available containers (adjust to your account)
  const [containers] = useState(["raw", "processed", "projects", "uploads", "issued"]);
  const [selectedContainer, setSelectedContainer] = useState(null);

  // Path segments within the selected container (excluding base for raw)
  const [currentPath, setCurrentPath] = useState([]);

  // Local selection buffer before adding to background queue
  ...
  /**
   * Helpers for RAW base prefix
   * RAW container layout: YYYY-MM-DD/username/...
   */
  ...
  /**
   * Enter folder (click)
   * `folderFullName` is a full prefix name (e.g., "2026-01-13/anonymous/myFolder")
   */
   ...
   * index === -1 → Home (no container)
   * index === 0 → inside container root
   * index > 0 → path slice
   */
  ...
  /**
   * Drop/select files into local buffer
   */
  ...
  /**
   * Add selected files to background queue (UploadQueueProvider)
   */
  const handleUploadClick = () => {
    ...
    const basePath =
      selectedContainer === "raw"
        ? getRawBasePath() + (currentPath.length ? currentPath.join("/") + "/" : "")
        : currentPath.length
          ? currentPath.join("/") + "/"
          : "";
  ...
  /**
   * SDK-based folder & file listing
   * IMPORTANT: For raw, always start listing under YYYY-MM-DD/username/
   */
  const fetchFolders = async (container, path) => {
    ...
    setFilesInFolder([]);
    try {
      const rawBase = container === "raw" ? getRawBasePath() : "";
      const effectivePrefix = normalizePrefix(rawBase + (path || ""));

      const containerClient = getContainerClient(container);
      const folderSet = new Set();
      const filesList = [];

      for await (const item of containerClient.listBlobsByHierarchy("/", { prefix: effectivePrefix })) {
        if (item.kind === "prefix") {
          folderSet.add(item.name.replace(/\/$/, "")); // strip trailing slash when displaying
  ...
  }, [selectedContainer, currentPath.join("/")]);

  /**
   * Display path for current upload target
   */
  ...
  /**
   * Create folder by placing a tiny placeholder blob under the prefix
   * For raw, forcibly include the YYYY-MM-DD/username base.
   */
  const createFolder = async (name) => {
    ...
    try {
      ...
      const folderPrefix = ensureSlash(basePrefix + name); // ends with /
      const blobPath = `${folderPrefix}.keep`; // tiny file to materialize folder
  ...
  /**
   * Rename folder: copy each blob to new prefix, then delete source
   * For raw, prefixes must include the base date/username.
   */
  const renameFolder = async (oldFolderFullName) => {
    const newName = prompt(
      "Enter new folder name:",
      oldFolderFullName.split("/").pop()
    );
    if (!newName) return;
    ...
    try {
      ...
      // Normalize old prefix and compute new prefix under the same parent
      const oldPrefix = ensureSlash(oldFolderFullName); // e.g. "2026-01-13/anonymous/foo/"
      const parentPrefix =
        selectedContainer === "raw"
          ? ensureSlash(getRawBasePath() + (currentPath.length ? currentPath.join("/") : ""))
          : ensureSlash(currentPath.length ? currentPath.join("/") : "");
      const newPrefix = parentPrefix + ensureSlash(newName); // e.g. ".../bar/"

      // Copy each blob to new prefix, then delete original
      for await (const item of containerClient.listBlobsFlat({ prefix: oldPrefix })) {
        ...
        // Server-side copy (requires CORS to allow PUT + x-ms-copy-source)
  ...
  const totalPages =
    Math.ceil(filteredFolders.length / itemsPerPage) || 1;
                  ...
                  {filesInFolder.length > 0 && (
                    <div className="mt-4">
                      ...
                      {filesInFolder.map((item, index) => (
                        <div
                          key={`file-${index}`}
                          className="flex items-center p-2 rounded-md text-fg/70"
                        >
      ...
      {queue.length > 0 && (
        <div className="mt-8 pt-4 border-t border-glass-border">
          ...
          <div className="mt-4 text-sm text-fg/80">
            Overall: {totalGlobalProgress}% ({globalProgress.uploadedCount}/{globalProgress.totalCount} files)
          </div>
        </div>
      )}
    </div>
