...
async function listAllBlobsFlat({ baseUrl, sasToken, container, query, signal, stopOnFirst = false, maxPages = 20, prefix = "" }) {
  if (!sasToken) {
  ...
  do {
    ...
    const url =
      ...
      `&maxresults=5000${markerParam}${prefixParam}&${sas}`;
...
/***** start of searchWithinContainer *****/
// ADD this new export after the existing searchAcrossContainers function:

export async function searchWithinContainer({
  baseUrl,
  sasToken,
  query = "",
  container,
  pathPrefix = "",       // e.g. "2025-12-09/anonymous/test" — scope to current folder
  searchFromRoot = false, // if true, ignore pathPrefix and search entire container
  onProgress,
  signal,
  maxPages = 20,
}) {
  ...
  // Use current path as prefix so results are scoped to where the user is.
  // If searchFromRoot is true, widen to the entire container.
  const prefix = searchFromRoot ? "" : (pathPrefix ? pathPrefix.replace(/\/?$/, "/") : "");

  onProgress?.({ container, found: 0, done: false, mode: "within" });
  ...
  try {
    ...
    onProgress?.({ container, found: matched.length, done: true, mode: "within", currentMatches: matched });
    return matched;
  } catch (err) {
    if (!signal?.aborted) {
      console.warn(`[containerSearch] Error searching within "${container}":`, err);
      onProgress?.({ container, found: 0, done: true, mode: "within", error: err });
    }
    return [];
  }
}

/***** end of searchWithinContainer   *****/
