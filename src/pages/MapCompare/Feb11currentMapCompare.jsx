...
useEffect(() => {
  ...
  (async () => {
    ...
    const worker = async (f) => { /* your existing worker unchanged */ };

    const onBatch = (partial) => {
      if (cancelled) return;
      setLeftBlobMarkers(partial);
      // progress: processed rows (including nulls filtered by your code) / total
    ...
    if (!cancelled) {
      ...
      setTimeout(() => setLeftMapLoading(false), 150); // small grace for smooth fade
    }
  })();
