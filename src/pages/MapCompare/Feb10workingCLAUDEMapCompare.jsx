...
/*
  const { filteredFolders, filteredFiles } = searchInContainer(files, folders, query);
 */ 
...
useEffect(() => {
  if (rightSource !== "blob") { setRightBlobMarkers([]); return; }
  ...
  (async () => {
    ...
    const onBatch = (partial) => { if (!cancelled) setRightBlobMarkers(partial); };
    const result = await processInBatches(currentList, 32, worker, onBatch);
    if (!cancelled) setRightBlobMarkers(result);
  })();
...
useEffect(() => {
  ...
  (async () => {
    ...
    const onBatch = (partial) => { if (!cancelled) setRightUploadMarkers(partial); };
    const result = await processInBatches(rightUploads, 32, worker, onBatch);
    if (!cancelled) setRightUploadMarkers(result);
  })();
