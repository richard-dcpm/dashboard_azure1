...
/* ------------------------------------------------------------- */
/* Inline UI Components (no extra files needed) */
/* ------------------------------------------------------------- */
            ...
            <div className="px-4 py-3 border-b border-white/20 bg-white/5">
              <div className="flex items-center gap-2 flex-wrap">
                {loadingContainers ? (
                  ...
                  (containers?.length ? containers : fallbackContainers).map((c) => (
                    <button
                      key={c}
    ...
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
      ...
      {/**files.map((file) => (
        <button
          ...
          className="group p-2 rounded-lg border border-gray-200 bg-white hover:shadow text-left"
        >
          
		  <div className="aspect-video w-full rounded bg-gray-50 overflow-hidden flex items-center justify-center">
            <img src={file.url} alt="" className="max-h-24 object-contain" loading="lazy" />
          </div>
...
// 1. Inside MapCompare component, add these navigation handlers:
const handleNavigate = (side, direction) => {
  if (side === "left") {
    const list = leftSource === "upload" ? leftUploads : leftBlob.files;
    const current = leftSource === "upload" ? leftUploadIndex : leftBlob.selectedIndex;
    ...
    if (leftSource === "upload") setLeftUploadIndex(nextIdx);
    else setLeftBlob(s => ({ ...s, selectedIndex: nextIdx, selectedFile: s.files[nextIdx] }));
  } else {
    const list = rightSource === "upload" ? rightUploads : rightBlob.files;
    const current = rightSource === "upload" ? rightUploadIndex : rightBlob.selectedIndex;
    if (list.length === 0) return;
    ...
    if (rightSource === "upload") setRightUploadIndex(nextIdx);
    else setRightBlob(s => ({ ...s, selectedIndex: nextIdx, selectedFile: s.files[nextIdx] }));
  }
};

// 2. Updated ThumbnailStrip Component to include Nav Buttons logic
...
export default function MapCompare() {
  ...
  // NEW: State to toggle Right Panel
  ...
  // Prevent move-event feedback loop while syncing maps
...
// ✅ LOGIC FIX: If user switches away from Blob → Upload, force-close explorer modal
useEffect(() => {
...
useEffect(() => {
  ...
  /**const toLatLngKey = (pos) => `${pos.lat.toFixed(6)},${pos.lng.toFixed(6)}`;**/
  const toLatLngKey = (pos) => `${pos.lat.toFixed(6)},${pos.lon.toFixed(6)}`;

// 1. Refined grouping logic to ensure markers with identical coordinates cluster together
/*
const groupByCoordinate = (items) => {
  const map = new Map();
  for (const it of items) {
    if (!it.pos) continue;
    // Rounding to 6 decimal places to ensure consistent keys for matching coordinates
    const key = `${it.pos.lat.toFixed(6)},${it.pos.lon.toFixed(6)}`;
    const existing = map.get(key);
    if (existing) {
      existing.urls.push(it.url);
    } else {
      map.set(key, { pos: it.pos, urls: [it.url] });
    }
  }
  return Array.from(map.values());
};
*/
// Group markers by identical coordinates, but keep full item info
const groupByCoordinate = (items) => {
  const map = new Map();
  for (const it of items) {
    if (!it.pos) continue;
    ...
    if (existing) {
      existing.items.push(it);     // ✅ keep whole item
    } else {
...
// Logic to build a popup containing thumbnails for clustered images
/*
const buildPopupHtml = (urls) => {
  const imgs = (urls || [])
    .slice(0, 4) // Limit to 4 previews in popup
    .map((u) => `<img src="${u}" style="width:100%; border-radius:4px; margin-bottom:4px;"/>`)
    .join("");
...
// UPDATED: Plotting logic for Left Map
// 2. Updated Left Map Plotting Logic
//useEffect(() => {
//  if (!leftMap.isReady) return;
  
  // Logic to determine which marker data to use
//  let markerData = [];
  
//  if (leftSource === "blob" && leftBlobMarkers.length > 0) {
//    markerData = leftBlobMarkers;
//  } else if (leftSource === "upload" && leftUploads.length > 0) {
    // Generate marker data for uploaded files if multiple exist
//    markerData = leftUploads.map((file, idx) => {
//      const url = URL.createObjectURL(file);
//      const gps = parseLatLonFromName(file.name); // Fallback to name parsing if EXIF isn't ready
//      return { pos: normalizePos(gps), url, name: file.name };
...
//        lat: g.pos.lat,
//        lon: g.pos.lon,
//        popupHtml: buildPopupHtml(g.urls),
//      })),
//      DEFAULT_CENTER,
//      DEFAULT_ZOOM
//    );
//  } else {
    // Single marker fallback for single selections or empty states
 //   leftMap.setMarker(
 //     leftMarkerPos, 
 //     DEFAULT_CENTER, 
 //     DEFAULT_ZOOM, 
 //     buildPopupHtml(leftSelectedUrl ? [leftSelectedUrl] : [])
 //   );
 // }
//}, [leftMap.isReady, leftBlobMarkers, leftUploads, leftMarkerPos, leftSelectedUrl, leftSource]);
  ...
  (async () => {
    ...
    if (leftSource === "blob" && leftBlobMarkers.length > 0) {
      ...
      const results = await Promise.all(
        leftUploads.map(async (file) => {
          let gps = null;

          try {
            gps = await extractGpsFromFile(file);   // ✅ EXIF first
          } catch (e) {}

          if (!gps) gps = parseLatLonFromName(file.name); // ✅ fallback

          const pos = normalizePos(gps);
          if (!pos) return null;

          return { pos, url: URL.createObjectURL(file), name: file.name };
        })
      );

      markerData = results.filter(Boolean);
    }
...
// UPDATED: Plotting logic for Right Map
// 3. Updated Right Map Plotting Logic
//useEffect(() => {
//  if (!rightMap.isReady) return;

//  let markerData = [];
  
//  if (rightSource === "blob" && rightBlobMarkers.length > 0) {
//    markerData = rightBlobMarkers;
//  } else if (rightSource === "upload" && rightUploads.length > 0) {
//    markerData = rightUploads.map((file) => {
//      const url = URL.createObjectURL(file);
//      const gps = parseLatLonFromName(file.name);
//      return { pos: normalizePos(gps), url, name: file.name };
//    }).filter(item => item.pos !== null);
//  }
...
//      DEFAULT_CENTER, 
//      DEFAULT_ZOOM, 
//      buildPopupHtml(rightSelectedUrl ? [rightSelectedUrl] : [])
//    );
//  }
//}, [rightMap.isReady, rightBlobMarkers, rightUploads, rightMarkerPos, rightSelectedUrl, rightSource]);
...
useEffect(() => {
  ...
  (async () => {
    ...
    if (rightSource === "blob" && rightBlobMarkers.length > 0) {
      ...
      const results = await Promise.all(
        rightUploads.map(async (file) => {
          let gps = null;

          try {
            gps = await extractGpsFromFile(file);
          } catch (e) {}
  ...
  rightSource,
	...
	// Add this central navigation handler inside MapCompare
	const handleStep = (side, direction) => {
	  ...
	  // Update State (This automatically syncs the ThumbnailStrip highlight)
	  if (isLeft) {
  ...
  const getGpsForBlobFile = async (file) => {
    try {
      const exif = await extractGpsFromUrl(file.url);
      if (exif) return exif;
    } catch (e) {}
    return parseLatLonFromName(file.name);
  };
/*
  useEffect(() => {
    if (leftSource !== "blob") {
    ...
    (async () => {
	  const results = await Promise.all(
		  currentList.map(async (f) => {
			const gps = await getGpsForBlobFile(f); // <-- MUST exist in this scope
			const p = normalizePos(gps);
			if (!p) return null;
      ...
      if (!cancelled) setLeftBlobMarkers(results.filter(Boolean));
    })();
...
*/
// LEFT blob -> markers (batched + progressive)
useEffect(() => {
  if (leftSource !== "blob") { setLeftBlobMarkers([]); return; }
  const currentList = leftBlob.files;
  let cancelled = false;

  (async () => {
    const worker = async (f) => {
      const cacheKey = f.url;                 // stable for blob files
    ...
    const onBatch = (partial) => { if (!cancelled) setLeftBlobMarkers(partial); };
    const result = await processInBatches(currentList, 16, worker, onBatch);
    if (!cancelled) setLeftBlobMarkers(result);
  })();
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!cancelled) setRightBlobMarkers(results.filter(Boolean));
    })();
	...
	// --- SYNC MAPS SNIPPET (FIXED) ---
	//useEffect(() => {
	//  if (!leftMap.isReady || !rightMap.isReady || !showRightPanel) return;

	//  const left = leftMap.mapInstance;
	//  const right = rightMap.mapInstance;
	...
	//	isSyncingRef.current = true;
		...
		// Use moveend/zoomend so we don't sync on every animation frame
	//	const center = source.getCenter();
	//	const zoom = source.getZoom();

	//	target.setView(center, zoom, { animate: false });

		// Release lock next tick so target’s resulting events don’t recurse
	//	requestAnimationFrame(() => {
	//	  isSyncingRef.current = false;
	//	});
	//  };

	//  const onLeftMoveEnd = () => sync(left, right);
	//  const onRightMoveEnd = () => sync(right, left);

	// left.on("moveend", onLeftMoveEnd);
	//  left.on("zoomend", onLeftMoveEnd);

	//  right.on("moveend", onRightMoveEnd);
	//  right.on("zoomend", onRightMoveEnd);

	//  return () => {
	//	left.off("moveend", onLeftMoveEnd);
	//	left.off("zoomend", onLeftMoveEnd);

	//	right.off("moveend", onRightMoveEnd);
	//	right.off("zoomend", onRightMoveEnd);
	//  };
	//}, [leftMap.isReady, rightMap.isReady, showRightPanel, leftMap.mapInstance, rightMap.mapInstance]);

	useEffect(() => {
		if (showRightPanel && rightMap.mapInstance) {
		  // We must wait for the browser to finish drawing the 2nd column
		  const resizeTimeout = setTimeout(() => {
			rightMap.mapInstance.invalidateSize();
		  }, 300); // 300ms ensures the grid transition is complete
		  
		  return () => clearTimeout(resizeTimeout);
		}
	  }, [showRightPanel, rightMap.mapInstance]);
	  
  // Add these handlers to navigate and keep thumbnails in sync
...
// Global cache to prevent redundant EXIF parsing and eliminate delays
const gpsCache = new Map();
const objectUrlCacheRef = React.useRef(new Map()); // prevent repeated createObjectURL
...
// ---- Batched async helper (limits concurrency implicitly by chunk size)
const processInBatches = async (items, batchSize, worker, onBatch) => {
  ...
  for (let i = 0; i < items.length; i += batchSize) {
    ...
    onBatch?.(out.slice());              // push a copy of what we have so far
    await new Promise(r => setTimeout(r, 0)); // yield to UI
  }
  return out;
};

// Optimized: Extract GPS data in batch with caching
const extractGpsDataBatch = async (items, sourceType) => {
  const promises = items.map(async (item) => {
    const cacheKey = sourceType === 'blob' ? item.url : item.name;
    
    // Return cached result if available
    if (gpsCache.has(cacheKey)) {
      return { ...gpsCache.get(cacheKey), item };
    }

    let gps = null;
    if (sourceType === 'blob') {
      // Use your existing exif service
      gps = await extractGpsFromUrl(item.url);
      if (!gps) gps = parseLatLonFromName(item.name);
    } else {
      // For uploads, check if GPS is already on the file object or extract it
      gps = item.gps || await extractGpsFromFile(item);
      if (!gps) gps = parseLatLonFromName(item.name);
    }

    if (gps && gps.lat && gps.lon) {
      const result = { lat: gps.lat, lon: gps.lon, name: item.name };
...
const plotMarkers = async (source, sourceType, cluster, singleLayer, map, defaultCenter, defaultZoom) => {
  if (!cluster || !map || !singleLayer) return;
  
  // LOGIC FIX: Clear single markers whenever clusters are being updated 
  // to prevent "lone pins" from appearing alongside the group.
  singleLayer.clearLayers();
  
  // Get items: For 'upload', we want all files in the collection to be plotted
  const itemsToPlot = sourceType === 'blob'
    ? (source.selected || []).map(path => (source.items || []).find(i => i.fullPath === path)).filter(Boolean)
    : (source.files || []); // For uploads, default to showing all uploaded files on the map
  ...
  const gpsData = await extractGpsDataBatch(itemsToPlot, sourceType);
  
  const positions = gpsData.map(gps => ({
    lat: gps.lat,
    lon: gps.lon,
    popupHtml: `
      <div style="text-align:center; min-width:150px;">
        <strong style="display:block; margin-bottom:5px;">${gps.name}</strong>
        <img src="${sourceType === 'blob' ? gps.item.url : URL.createObjectURL(gps.item)}" 
             style="width:100%; border-radius:4px; border:1px solid #ccc;"/>
      </div>`
  }));
  
  // Use the updated cluster function from mapLogic.js
  updateMarkersCluster(map, cluster, positions, defaultCenter, defaultZoom);
};
/*
useEffect(() => {
  ...
  (async () => {
    ...
    if (!cancelled) setLeftUploadMarkers(results.filter(Boolean));
  })();
...
*/
// LEFT uploads -> markers (batched + progressive)
useEffect(() => {
  if (leftSource !== "upload" || leftUploads.length === 0) { setLeftUploadMarkers([]); return; }
  ...
  (async () => {
    ...
    const onBatch = (partial) => { if (!cancelled) setLeftUploadMarkers(partial); };
    const result = await processInBatches(leftUploads, 16, worker, onBatch);
    if (!cancelled) setLeftUploadMarkers(result);
  })();
...
useEffect(() => {
  ...
  (async () => {
    ...
    if (!cancelled) setRightUploadMarkers(results.filter(Boolean));
  })();
  ...
  return (
    <div className="p-8 bg-white/10 backdrop-blur-xl rounded-xl shadow-2xl max-w-7xl mx-auto mt-8 border border-white/20">
      ...
      {/* Grid shifts between grid-cols-2 and grid-cols-1 */}
                ...
                <>
				  ...
				  {leftSelectedUrl && (
					  <div className="relative group mt-3 w-full overflow-hidden rounded-xl border border-white/20 shadow-lg">
						...
						{/* Overlay Nav Buttons for the image itself */}
                  ...
                  <>
                    ...
                    {rightSelectedUrl && (
					  <div className="relative group mt-3 w-full overflow-hidden rounded-xl border border-white/20 shadow-lg bg-black/10">
						{/* Container ensures consistent sizing across both panels */}
						<div className="w-full aspect-[4/3] bg-black/20 flex items-center justify-center">
						  <img 
	 ...
	 {/* Map Grid Section: Conditional Rendering for Right Map */}
      <div className={`grid ${showRightPanel ? "grid-cols-2" : "grid-cols-1"} gap-4`}>
        <div ref={leftMapRef} className="rounded-lg border border-white/20" style={{ height: "400px", width: "100%", position: "relative" }} />
        {showRightPanel && (
          <div ref={rightMapRef} className="rounded-lg border border-white/20" style={{ height: "400px", width: "100%", position: "relative" }} />
        )}
      </div>
