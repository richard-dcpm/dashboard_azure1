...
*/
 ...
 /* */
	...
	/* newly added feb 26 */
	React.useEffect(() => {
		// ✅ Abort the previous search IMMEDIATELY on every keystroke
		abortRef.current?.abort();
		...
		// Create the new controller now so it's ready before the timer fires
		const ctrl = new AbortController();
		abortRef.current = ctrl;

		// Debounce: wait for user to stop typing before firing the network calls
		const timer = setTimeout(async () => {
		  if (ctrl.signal.aborted) return; // user typed again before timer fired

		  setSearching(true);
		  setSearchResults([]);
		  setSearchStatus({ container: "…", found: 0, done: false });
		  ...
		  try {
			const matches = await searchAcrossContainers({
			  baseUrl,
			  sasToken,
			  query: query.trim(),
			  containers: ordered,
			  signal: ctrl.signal,
			  onProgress: ({ container, found, done, error, currentMatches }) => {
				if (ctrl.signal.aborted) return;

				setSearchStatus({ container, found, done, error });

				// Stream: update displayed results as each container reports in
				if (currentMatches) {
...
useEffect(() => {
  ...
  if (leftMap.plotMode === "individual") {
    // One pin per image — no grouping, hook will spread duplicates
    leftMap.setMarkers(
      markerData.map((m) => ({
        lat: m.pos.lat,
        lon: m.pos.lon,
        popupHtml: buildPopupHtml([m]),   // single-image popup
      })),
      DEFAULT_CENTER,
      DEFAULT_ZOOM,
      "individual"                        // ← tell hook which layer to use
    );
  } else {
    // Cluster mode — group same-coordinate images into one pin
    const grouped = groupByCoordinate(markerData);
    leftMap.setMarkers(
      grouped.map((g) => ({
        lat: g.pos.lat,
  ...
  leftUploadMarkers,
  leftSource,
  leftMap.plotMode,   // ← re-plot whenever the toggle changes
]);
...
useEffect(() => {
  ...
  if (rightMap.plotMode === "individual") {
    rightMap.setMarkers(
      markerData.map((m) => ({
        lat: m.pos.lat,
        lon: m.pos.lon,
  ...
  rightUploadMarkers,
  rightSource,
  rightMap.plotMode,  // ← re-plot whenever the toggle changes
]);
  ...
  return (
	...
	<div className={`grid ${showRightPanel ? "grid-cols-2" : "grid-cols-1"} gap-4`}>
	  ...
	  <div style={{ position: "relative", height: "400px", width: "100%" }}>
		...
		{/* Map Type Toggle - Left */}
		<div style={{ position: "absolute", top: "10px", right: "10px", zIndex: 1000 }}>
		  <button
			onClick={() => leftMap.toggleMapType?.()}
			className="px-3 py-1.5 rounded-lg border border-white/40 bg-white/90 backdrop-blur text-gray-800 shadow-lg hover:bg-white transition flex items-center gap-2 text-sm font-medium"
			title="Toggle map type"
		  >
			{leftMap.mapType === "road" ? (
			  <>
		  ...
		  {/* ✅ NEW – individual / cluster plot-mode toggle */}
			<button
			  onClick={() => leftMap.togglePlotMode?.()}
			  className="px-3 py-1.5 rounded-lg border border-white/40 bg-white/90 backdrop-blur text-gray-800 shadow-lg hover:bg-white transition flex items-center gap-2 text-sm font-medium"
			  title="Toggle pin plot mode"
			>
			  {leftMap.plotMode === "cluster" ? (
				<>
				  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
	  ...
	  {showRightPanel && (
		<div style={{ position: "relative", height: "400px", width: "100%" }}>
		  ...
		  {/* Map Type Toggle - Right */}
		  <div style={{ position: "absolute", top: "10px", right: "10px", zIndex: 1000 }}>
			<button
			  onClick={() => rightMap.toggleMapType?.()}
			  className="px-3 py-1.5 rounded-lg border border-white/40 bg-white/90 backdrop-blur text-gray-800 shadow-lg hover:bg-white transition flex items-center gap-2 text-sm font-medium"
			  title="Toggle map type"
			>
			  {rightMap.mapType === "road" ? (
				<>
				  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
					<rect x="3" y="3" width="18" height="18" rx="2"/>
					<path d="M3 9h18M9 3v18"/>
			...
			{/* ✅ NEW – individual / cluster plot-mode toggle */}
			<button
			  onClick={() => rightMap.togglePlotMode?.()}
			  className="px-3 py-1.5 rounded-lg border border-white/40 bg-white/90 backdrop-blur text-gray-800 shadow-lg hover:bg-white transition flex items-center gap-2 text-sm font-medium"
			  title="Toggle pin plot mode"
			>
			  {rightMap.plotMode === "cluster" ? (
				<>
				  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
					<circle cx="12" cy="12" r="3"/><circle cx="5" cy="5" r="2"/><circle cx="19" cy="5" r="2"/>
					<circle cx="5" cy="19" r="2"/><circle cx="19" cy="19" r="2"/>
